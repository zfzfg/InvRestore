    ...
    private void handleBackup(CommandSender sender, String[] args) {
        ...
        if (args[1].equalsIgnoreCase("all")) {
            ...
            for (Player player : Bukkit.getOnlinePlayers()) {
                ...
                // Only send message if notifications are enabled
                if (plugin.getConfig().getBoolean("notify-on-backup", true)) {
                    if (plugin.getConfig().getBoolean("notify-ops-only", true)) {
                        if (sender.isOp()) {
                        ...
                        if (sender.hasPermission("inventorybackup.notify") || sender.isOp()) {
                            String message = plugin.getMessage("backup-all-created")
                                    .replace("{count}", String.valueOf(count));
                            sender.sendMessage(message);
                        }
